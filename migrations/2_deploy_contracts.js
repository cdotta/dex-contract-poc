...
module.exports = async function (deployer, _network, accounts) {
  ...
  const contracts = [Dai, Bat, Rep, Zrx, Dex];
  await Promise.all(contracts.map((contract) => deployer.deploy(contract)));
  const [dai, bat, rep, zrx, dex] = await Promise.all(
    contracts.map((contract) => contract.deployed()),
  );
  await Promise.all([
  ...
  await Promise.all(
    [trader1, trader2, trader3, trader4].map((trader) =>
      Promise.all(
        [dai, bat, rep, zrx].map((token) => seedTokenBalance(token, trader)),
      ),
    ),
  );
